...
async function consulDiagnostic() {
  ...
  const consul = new Consul('http://192.168.1.68');
  const serviceName = 'cosy-service-test';
...
async function testConsulEndpoints() {
  ...
  const baseUrl = 'http://192.168.1.68:8500';
  const endpoints = [
    '/v1/status/leader',
    '/v1/status/peers',
    '/v1/agent/self',
    '/v1/catalog/services',
    '/v1/catalog/service/cosy-service-test',
    '/v1/health/service/cosy-service-test',
  ];
...
async function main() {
  ...
  if (result) {
    ...
    console.log('\n🔧 可能的解决方案:');
    console.log('1. 检查测试环境的TTS服务是否正在运行');
