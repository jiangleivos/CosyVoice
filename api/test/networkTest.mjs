...
async function testNetworkConnection() {
  ...
  try {
    await execAsync(`curl -s --connect-timeout 5 http://${testHost}:${consulPort}/v1/status/leader`);
...
async function getNetworkInfo() {
  ...
  try {
    ...
    // 检查网络接口状态
    const { stdout: interfaceInfo } = await execAsync("ifconfig | grep -E '^[a-z]|status:'");
    console.log(`${fn} 🔌 网络接口状态:`);
    console.log(interfaceInfo.trim());
  } catch (error) {
...
async function main() {
  ...
  if (result) {
    ...
    console.log('\n🔧 可能的解决方案:');
    console.log('1. 检查192.168.1.68服务器是否启动');
    console.log('2. 检查防火墙设置');
    console.log('3. 检查VPN连接（如果需要）');
    console.log('4. 确认测试环境网络配置');
  }
}
